 ...
 */
export function allocateChests(maze: Maze, rng: () => number): ChestAllocation {
  ...
  const eligible = maze.deadEnds.filter(
    (c) =>
      !(c.x === maze.entrance.x && c.y === maze.entrance.y) &&
  ...
  const armoryPool = shuffle(
    remaining.filter((c) => maze.bfsDistance(c) >= armoryThreshold),
    rng,
  );
  const armoryCells = armoryPool.slice(0, armoryTarget);

  // Supply: random from everything else.
  const supplyPool = shuffle(
    remaining.filter(
      (c) => !armoryCells.some((a) => a.x === c.x && a.y === c.y),
    ),
    rng,
  );
...
export function lootTierForFloor(floorNum: number): LootTier {
