...
let started = false;
export function startMonsterScheduler() {
    if (started)
        return;
    started = true;
    ...
    world.afterEvents.entityDie.subscribe((ev) => {
        try {
            clearRuntime(ev.deadEntity.id);
        }
        catch { /* ignore */ }
    });
    world.afterEvents.entityRemove.subscribe((ev) => {
        try {
            clearRuntime(ev.removedEntityId);
        }
        catch { /* ignore */ }
    });
}
function tickAll() {
    ...
    for (const e of entities) {
        let behavior;
        try {
            behavior = e.getDynamicProperty("trickymaze:behavior");
        }
        catch {
            continue;
        }
        ...
        try {
            ...
            console.warn(`[TrickyMaze] tick '${behavior}' on ${e.id} threw: ${String(err)}`);
        }
    }
}
export function monsterTickIntervalTicks() {
